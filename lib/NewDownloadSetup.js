...
Setup_New_Download.prototype.execute = function(callback) {
	...
	cParams.headUrl = cParams.headUrl || cParams.url;

	this.tasks = {

		//HTTP
		'http-head': ['file-handle', function(callback, results) {
			u.executor(HeadRequest, cParams.headUrl, cParams)(callback);
		}],
		...
		function(callback, results) {
			u.executor(DownloadValidator,
				results['file-handle'],
				results['meta-read'].threads)(callback);
		}],
